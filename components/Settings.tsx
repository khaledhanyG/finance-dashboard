        ...
        <nav className="space-y-1">
          {sidebarTabs.filter(t => t.show).map((tab) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg transition-all ${activeTab === tab.id ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-200'
                }`}
            >
      ...
      <div className="flex-1 p-8 overflow-y-auto max-h-[700px]">
        ...
        {activeTab === 'employees' && isAdmin && (
          <EmployeeManager
            ...
            onAdd={(name, empNo, deptId, salary, nationality, active, startDate, endDate) =>
              addItem('employees', { id: `emp-${Date.now()}`, name, employeeNumber: empNo, departmentId: deptId, salary, nationality, isActive: active, startDate, endDate })}
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/users', {
        ...
        body: JSON.stringify({ id: form.id, name: form.name, password: form.password, email: form.email })
      });
      ...
      onUpdate({ ...updatedUser, password: form.password }); // Include password in local state
          ...
          <tbody>
            {users.map(u => (
              <tr key={u.id} className="border-t border-slate-50 hover:bg-slate-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider ${u.role === 'admin' ? 'bg-indigo-100 text-indigo-700' :
...
const EmployeeManager: React.FC<{
  ...
  onAdd: (n: string, eno: string, d: string, s: number, nat: string, act: boolean, sd?: string, ed?: string) => void,
  onUpdate: (e: Employee) => void,
  onRemove: (emp: Employee) => void
}> = ({ items, departments, onAdd, onUpdate, onRemove }) => {
  const [form, setForm] = useState({ name: '', employeeNumber: '', deptId: '', salary: '', nationality: '', isActive: true, startDate: '', endDate: '' });
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    if (form.name && form.deptId && form.employeeNumber) {
      onAdd(form.name, form.employeeNumber, form.deptId, Number(form.salary), form.nationality, form.isActive, form.startDate, form.endDate);
      setForm({ name: '', employeeNumber: '', deptId: '', salary: '', nationality: '', isActive: true, startDate: '', endDate: '' });
    }
  };
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 p-4 bg-slate-50 rounded-xl border border-slate-100">
        ...
        <div className="flex gap-2">
          <input type="text" placeholder="Start Date" onFocus={(e) => e.target.type = 'date'} onBlur={(e) => e.target.type = 'text'} value={form.startDate} onChange={e => setForm({ ...form, startDate: e.target.value })} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm" />
          <input type="text" placeholder="End Date" onFocus={(e) => e.target.type = 'date'} onBlur={(e) => e.target.type = 'text'} value={form.endDate} onChange={e => setForm({ ...form, endDate: e.target.value })} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm" />
        </div>
                ...
                <td className="py-3 px-4">
                  {editingId === emp.id ? (
                    <div className="flex gap-1 flex-col">
                      <input type="date" className="border rounded px-2 py-1 text-xs" value={emp.startDate || ''} onChange={(e) => onUpdate({ ...emp, startDate: e.target.value })} />
                      <input type="date" className="border rounded px-2 py-1 text-xs" value={emp.endDate || ''} onChange={(e) => onUpdate({ ...emp, endDate: e.target.value })} />
                    </div>
                  ) : (
                    <div className="text-[10px] text-slate-500">
                      {emp.startDate && <div><i className="fas fa-play text-emerald-400 w-4"></i> {emp.startDate}</div>}
                      {emp.endDate && <div><i className="fas fa-stop text-rose-400 w-4"></i> {emp.endDate}</div>}
                    </div>
                  )}
                </td>
                ...
                <div className="flex flex-wrap gap-2">
                  {categories.filter(c => c.groupId === group.id).map(cat => (
                    <div key={cat.id} className="flex items-center gap-2 bg-slate-50 px-3 py-1 rounded-full text-[10px] font-bold text-slate-600 group hover:bg-slate-100 transition-all border border-slate-200">
                      {editingCatId === cat.id ? (
                        <div className="flex items-center gap-1">
                          <input autoFocus className="bg-transparent border-b border-indigo-200 outline-none w-24 text-indigo-700" value={cat.name} onChange={(e) => onUpdateCategory({ ...cat, name: e.target.value })} onKeyDown={e => e.key === 'Enter' && setEditingCatId(null)} />
                          <select
                            value={cat.groupId}
                            onChange={(e) => onUpdateCategory({ ...cat, groupId: e.target.value })}
                            className="w-20 text-[9px] border-none bg-transparent outline-none text-slate-400 font-normal hover:bg-slate-200 rounded cursor-pointer"
                          >
                            {groups.map(g => <option key={g.id} value={g.id}>{g.name.substring(0, 10)}</option>)}
                          </select>
                        </div>
