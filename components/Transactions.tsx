...
export const Transactions: React.FC<TransactionsProps> = ({
  ...
  const startEditExpense = (expense: ExpenseEntry) => {
    ...
    setDivideAmongAll(expense.isShared || false);
    setExpForm({
      date: expense.date,
      journalNo: expense.journalNo,
      categoryId: expense.categoryId,
      departmentId: expense.departmentId || '',
      employeeId: expense.employeeId || '',
      amount: expense.amount.toString(),
      amountPaid: expense.amountPaid.toString(),
      description: expense.description.replace(' (Shared Operational Cost)', '')
    });
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  const handleSubmitExpense = (e: React.FormEvent) => {
    ...
    if (editingExpId) {
      onUpdateExpense({
        id: editingExpId,
        date: expForm.date,
        journalNo: expForm.journalNo,
        categoryId: expForm.categoryId,
        departmentId: divideAmongAll ? '' : expForm.departmentId,
        employeeId: divideAmongAll ? null : (expForm.employeeId || null),
        amount: totalAmount,
        amountPaid: paid,
        remainingAmount: remaining,
        description: divideAmongAll ? `${expForm.description} (Shared Operational Cost)` : expForm.description,
        isShared: divideAmongAll
      });
      cancelEditExpense();
      return;
    }
    ...
    if (divideAmongAll) {
      // Create a single shared expense entry
      ...
      onAddExpense({
        ...
        categoryId: expForm.categoryId,
        departmentId: '', // Shared expenses don't belong to a specific department initially
        employeeId: null,
        amount: totalAmount,
        amountPaid: paid,
        remainingAmount: remaining,
        description: `${expForm.description} (Shared Operational Cost)`,
        isShared: true
      });
      if (remaining > 0) onAddOutstanding({ id: `out-${Date.now()}`, expenseId: entryId, date: expForm.date, amount: remaining, departmentId: 'SHARED', description: `Balance for: ${expForm.description} (Shared)` });
    } else {
  ...
  const filteredExpenses = useMemo(() => {
    ...
    let list = state.expenseEntries.slice().reverse().filter(e =>
    ...
    if (expenseStartDate) list = list.filter(e => e.date >= expenseStartDate);
    if (expenseEndDate) list = list.filter(e => e.date <= expenseEndDate);
    return list;
  }, [state.expenseEntries, state.departments, state.expenseCategories, expenseSearch, expenseStartDate, expenseEndDate]);
            ...
            {mode === 'expense' ? (
              <form onSubmit={handleSubmitExpense} className="space-y-4">
                ...
                <label className="flex items-center gap-2 p-2 bg-slate-50 rounded border text-[10px] font-bold uppercase cursor-pointer">
                  <input type="checkbox" checked={divideAmongAll} onChange={e => setDivideAmongAll(e.target.checked)} className="rounded text-indigo-600" /> Shared Operational Expense (Distribute dynamically)
                </label>
                {!divideAmongAll && (
                ...
                <button type="submit" className={`w-full ${editingExpId ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-emerald-600 hover:bg-emerald-700'} text-white py-3 rounded-xl shadow-lg transition-all font-black uppercase tracking-widest text-xs`}>
                  {editingExpId ? 'Update Expense' : 'Post Expense'}
                </button>
                {editingExpId && (
                  <button type="button" onClick={cancelEditExpense} className="w-full bg-slate-400 text-white py-2 rounded-xl shadow transition-all font-bold uppercase tracking-widest text-[10px] hover:bg-slate-500">
                    Cancel Edit
                  </button>
                )}
              </form>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="p-4 border-b bg-slate-50 flex justify-between items-center">
            ...
            {mode === 'income' ? (
              ...
              <div className="flex gap-2">
                 <input type="text" placeholder="Search..." value={expenseSearch} onChange={e => setExpenseSearch(e.target.value)} className="text-[10px] px-2 py-1 bg-white border rounded outline-none w-24 mr-2" />
                 <input type="date" value={expenseStartDate} onChange={e => setExpenseStartDate(e.target.value)} className="text-[10px] px-2 py-1 bg-white border rounded outline-none" placeholder="Start Date" />
                 <span className="text-slate-400 self-center">-</span>
                 <input type="date" value={expenseEndDate} onChange={e => setExpenseEndDate(e.target.value)} className="text-[10px] px-2 py-1 bg-white border rounded outline-none" placeholder="End Date" />
              </div>
            ) : (
              ...
              <tbody>
                ...
                {mode === 'expense' && filteredExpenses.map(e => (
                  <tr key={e.id} className="border-b hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs font-bold text-indigo-400 uppercase">{e.isShared ? 'SHARED' : state.departments.find(d => d.id === e.departmentId)?.name}</td>
                    <td className="px-4 py-3 text-right font-black">{e.amount.toLocaleString()}</td>
                    <td className="px-4 py-3 text-right whitespace-nowrap">
                      <button onClick={() => startEditExpense(e)} className="text-slate-300 hover:text-indigo-600 mr-3"><i className="fas fa-edit"></i></button>
                      <button onClick={() => onDeleteExpense(e.id)} className="text-slate-300 hover:text-rose-600"><i className="fas fa-trash-can"></i></button>
                    </td>
                  </tr>
                ))}
